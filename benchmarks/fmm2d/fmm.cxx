...
int main(int argc, char ** argv) {
  ...
  for (size_t b=0; b<bodies.size(); b++) {                      // Loop over bodies
    for (int d=0; d<2; d++) {                                   //  Loop over dimension
      ...
      bounds.minX[d] = std::min(bounds.minX[d],bodies[b].X[d]); //   Update min bounds
      bounds.maxX[d] = std::max(bounds.maxX[d],bodies[b].X[d]); //   Update max bounds
...
#if USE_XITAO
  printf("Total number of steals: %ld\n", tao_total_steals);
#endif 
  return 0;
}
